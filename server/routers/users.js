...
router.get('/options', async (req, res) => {
    try {
        const options = await Options.findOne({},{googleKey:1,callCoin:1,instagramCoin:1,creditText:1,creditUrl:1,helpUrl:1})
        res.send(options)
    } catch (e) {
...
//canMake
router.get('/canMake',auth, async (req, res) => {
    try {
        const options = await Options.findOne({},{googleKey:1,callCoin:1,instagramCoin:1,creditText:1,creditUrl:1,helpUrl:1,messageCoin:1,imageCoin:1})
        let user = await User.findById(req.user._id)
        let type = req.query.type || "0"

        if (type == "0") {
            let coins = user.coins
            let cost = options.messageCoin || 1

            if (coins >= cost) {
                res.send({canMake:true})
            }else{
                res.send({canMake:false})
            }
        }else if (type == "1") {
            let coins = user.coins
            let cost = options.imageCoin || 1
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        user.coins = options.defaultCoin
...
async function sendMessageAfter(user,req){
    ...
    if (options.messageDelay && options.messageDelay > 0) {
        //convert messageDelay min to ms set delayMS
        delayMs = options.messageDelay * 60000
    }
    ...
    try {
      ...
      if (!user) {
        ...
        let newUser = new User({
          ...
          sur_name:sur_name,
          email: username,
          coins:options.defaultCoin || defaultCoin, 
