...
router.get("/fix-host",async (req, res) => {

    let gifts = await Gift.find({})

    let oldHost = "213.238.181.236"
    let newHost = "213.238.181.226"

    let users = await User.find({})
    for (let i = 0; i < users.length; i++) {
        let user = users[i]
        if (user.pp) {
            user.pp = user.pp.replace(oldHost,newHost)
        }
        
        if (user.images != null && user.images.length > 0) {
            var newImages = []
            for (let j = 0; j < user.images.length; j++) {
                let image = user.images[j]
                if (image) {
                    image = image.replace(oldHost,newHost)
...
//20 günden önceki mesajları  sil ve mesajı olmayan dialogları sil
router.delete('/delete-old-messages', adminAuth, async (req, res) => {
    try {
        let date = new Date()
        date.setDate(date.getDate() - 20)

        await Message.deleteMany({ createdAt: { $lt: date } })

        //lookup messages and lastMessage if messages or lastMessage null remove it
        let dialogs = await MessageDialog.aggregate([
            {
                $lookup: {
                    from: "messages",
                    localField: "_id",
        ...
        await MessageDialog.deleteMany({ _id: { $in: dialogIds } })
...
router.get('/calculate-receiver', adminAuth, async (req, res) => {
    try {
        let project = {_id:1}
        const all = "0"//Herkese gönder
        const vip = "1"//Şuanda vip olanlar
        const noVip = "2"//Hiç vip almayanlar
        const vipEnd = "3"//Daha önce vip almış ve bitmiş olanlar
        const coinLess = "4"//Jetonu belirtilen sayıdan düşük olanlar
        const coinMore = "5"//Jetonu belirtilen sayıdan yüksek olanlar
        const talked = "6"//Daha önce konuştukları
        const notTalked = "7"//Daha önce konuşmadıkları
        
        let profileId = req.query._id
        ...
        if (type == all) {//Herkese gönder
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true}},project)
            console.log(getCount(users));
            result.count = getCount(users)
        }else if(type == vip){//Şuanda vip olanlar
            //vipEnd not null and vipEnd greater than now
            let currentDate = new Date();
                let users = await User.find({
                admin: { $ne: true },
                fake: { $ne: true },
                mod: { $ne: true },
                vipEnd: { $ne: null, $gt: currentDate }
                },project);

            result.count = getCount(users)
        }else if(type == noVip){//Hiç vip almayanlar
            //vipEnd null
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true},vipEnd:null},project)
            result.count = getCount(users)
        }else if(type == vipEnd){//Daha önce vip almış ve bitmiş olanlar
            //vipEnd not null and vipEnd less than now
            let currentDate = new Date();
            let users = await User.find({
            admin: { $ne: true },
            fake: { $ne: true },
            mod: { $ne: true },
            vipEnd: { $ne: null, $lt: currentDate }
            },project);

            result.count = getCount(users)
        }else if(type == coinLess){//Jetonu belirtilen sayıdan düşük olanlar
            //coin less than coin
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true},coins:{$lt:coin}},project)
            result.count = getCount(users)
        }else if(type == coinMore){//Jetonu belirtilen sayıdan yüksek olanlar
            //coin greater than or equal coin
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true},coins:{$gte:coin}},project)
            result.count = getCount(users)
        }else if(type == talked){//Daha önce konuştukları
            //find dialog
            let dialogs = await MessageDialog.find({$or:[{user1:profileId},{user2:profileId}]})
            let dialogUsers = []
            for (let i = 0; i < dialogs.length; i++) {
                let dialog = dialogs[i]
                let user = dialog.user1 == profileId ? dialog.user2 : dialog.user1
                dialogUsers.push(user)
            }
            //find users
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true},_id:{$in:dialogUsers}},project)
            result.count = getCount(users)
        }else if(type == notTalked){//Daha önce konuşmadıkları
            //find dialog
            let dialogs = await MessageDialog.find({$or:[{user1:profileId},{user2:profileId}]})
            let dialogUsers = []
            for (let i = 0; i < dialogs.length; i++) {
            ...
            //find users
            let users = await User.find({admin:{$ne:true},fake:{$ne:true},mod:{$ne:true},_id:{$nin:dialogUsers}},project)
            result.count = getCount(users)        
        }else{
            result.count = 0
        }
...
router.get('/send-all', adminAuth, async (req, res) => {
    try {
        let project = {_id:1}
        const all = "0"//Herkese gönder
        const vip = "1"//Şuanda vip olanlar
...
async function sendAll(req,villSend) {
    console.log(villSend);
    for (let i = 0; i < villSend.length; i++) {
        const element = villSend[i];
        send(req,element)
    }
}
...
async function send(req,user){
    let profileId = req.query._id
    let rawMessage = req.query.message
    console.log(req.query);
    try {   
      if (rawMessage != null) {
            let sender = profileId
            let profile = user._id
            const dialog = await MessageDialog.findOne({ $or: [{ user1: sender, user2: profile }, { user1: profile, user2: sender }] })
...
router.put('/add-vips', adminAuth, async (req, res) => {
    try {
        const moment = require('moment')

        // Find a user with the given ID and select only the 'coins' field
        let user = await User.findById(req.body._id, { coins: 1 })

        // Get the number of days for the VIP period from the request body
        let day = Number(req.body.day)

        // Create Moment.js objects for the VIP start and end dates
        let vipStart = moment().toDate()
        let vipEnd = moment().add(day, 'days').toDate()

        // Update the user object with the new VIP start and end dates
        user.vipStart = vipStart
        user.vipEnd = vipEnd

        // Save the changes to the database and send the updated user object in the response
        await user.save()
        res.send(user)
...
router.put('/update-options', adminAuth, getOptions, async (req, res) => {
    try {
        ...
        if (options == null) {
            ...
            options.callCoin = req.body.callCoin
            ...
            options.helpUrl = req.body.helpUrl
            await options.save()
        }
        res.send(options)
    } catch (error) {
...
router.get('/available-users', modAuth, async (req, res) => {
    try {
       ...
       let  query = {admin:{$ne:true},fake:true,mod:{$ne:true}}
        //console.log(query);
...
router.get('/dialogs', modAuth, async (req, res) => {
    try {
        ...
        const withAll = Boolean((req.query.all || "").replace(/\s*(false|null|undefined|0)\s*/i, ""))
        //console.log("dialogs");
        //console.log(req.query);
        var limit = 10
        var skip = 0
...
router.get('/dialogs-page', modAuth, async (req, res) => {
    try {
        ...
        MessageDialog.countDocuments(query).then(result => {
...
router.post('/send-notif', modAuth, async (req, res) => {
    ...
    try {
        ...
        if (user == "all") {
            ...
            let userObj = await User.findById(user, { ftoken: 1 })
            console.log(userObj);
